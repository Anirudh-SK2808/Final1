  ...
  return (
    <div className="col-md-3 col-lg-2 sidebar p-3 bg-success text-white min-vh-100 d-flex flex-column">
      ...
      <div className="nav flex-column">
        ...
        <button className={`btn btn-dark text-white text-start nav-link mb-2 ${activeSection === "assigned" ? "active" : ""}`} onClick={() => setActiveSection("assigned")}>
          Assigned Campaigns
        </button>
...
// ModifyCampaign Component (unchanged)
const ModifyCampaign = ({ campaign, onDelete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:3000/campaigner/campaign/view/${campaign.id}`, formData, {
  ...
  return (
    <div>
      <button className="btn btn-warning btn-md" onClick={() => setShowForm(true)}>Modify</button>
              ...
              <div className="modal-body">
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="modal-footer">
                    ...
                    <button type="button" className="btn btn-danger" onClick={handleDelete} disabled={campaign.assigned_to}>Delete</button>
...
const CampaignerDashboard = () => {
  ...
  const fetchCampaigns = async () => {
    ...
    try {
      const res = await axios.get("http://localhost:3000/campaigner/campaign", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setCampaigns([...res.data.assigned_campaigns, ...res.data.unassigned_campaigns]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3000/campaigner/campaign/view", { name, description, start_date: startDate, end_date: endDate, goal, contact }, { headers: { Authorization: `Bearer ${token}` } });
      setName("");
      setDescription("");
      setStartDate("");
      setEndDate("");
                ...
                <ul className="list-group">
                  {campaignerInfo.new_requests.map((req, idx) => (
                    <li key={idx} className="list-group-item d-flex justify-content-between align-items-center">
                      <span>{req.name} - {req.campaign_id}</span>
                      <div>
                        <button className="btn btn-primary mx-2" onClick={() => handleRequestAction(req.campaign_id, req.volunteer_id, "approve", req.requirements)}>Approve</button>
                        <button className="btn btn-warning mx-2" onClick={() => handleRequestAction(req.campaign_id, req.volunteer_id, "update", prompt("Enter new requirements:"))}>Update</button>
                        <button className="btn btn-danger" onClick={() => handleRequestAction(req.campaign_id, req.volunteer_id, "delete")}>Delete</button>
                      </div>
                    </li>
                  ))}
                </ul>
